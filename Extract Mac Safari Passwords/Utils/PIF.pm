...
#   apicredential =>	{ typeNum => '112', typeName => '112',						title => 'API Credential' },
#   medicalrecord =>	{ typeNum => '113', typeName => '113',						title => 'Medical Record' },
...
sub create_pif_file {
    my ($recordlist, $outfile, $types) = @_;

    # validate pif table in case a module has added (incorrect) entries via add_new_field()
    check_pif_table();		
    ...
    my $ntotal = 0;
    verbose 'Export:';
    for my $type (sort { @{$recordlist->{$a}} <=> @{$recordlist->{$b}} } keys %$recordlist) {
	next if $types and not exists $types->{lc $type};

	my $n;
	for my $card (@{$recordlist->{$type}}) {
	...
	$ntotal += $n;
	verbose sprintf " %4d %s %s", $n, $typeMap{$type}->{'title'}, pluralize('item', $n);
    }

    verbose sprintf " %4d total %s", $ntotal, pluralize('item', $ntotal);
    ...
    $ntotal and 
	verbose "You may now import into 1Password the file:\n    $outfile"
}
