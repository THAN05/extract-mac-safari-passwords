...
sub normalize_card_data {
	...
	# Sanity check: look for multiple matches to detect inadvertent duplicate cfs field keys
	if (defined $card_field_specs->{$type}{'fields'}) {
	    for (my $cfs_index = 0; $cfs_index < @{$card_field_specs->{$type}{'fields'}}; $cfs_index++) {
		my $cfs = $card_field_specs->{$type}{'fields'}[$cfs_index];
		if (ref $cfs->[CFS_MATCHSTR] eq 'Regexp') {
			...
			push @found, { cfs => $cfs, index => $cfs_index };
		    }
		}
	...
	if (@found) {
	    ...
	    my ($cfs, $cfs_index) = ( $found[0]->{'cfs'}, $found[0]->{'index'} );
		...
		my $kind;
		# Hashref value indicates that attributes have been passed
		if (ref $value eq 'HASH') {
		    $kind  = $value->{'kind'};
		    $value = $value->{'value'};
		}
		$h = {
		    order	=> $added_field_order++,
		    inkey	=> $inkey,
		    value	=> $value,
		    valueorig	=> $value,
		...
		add_new_field($h->{'outtype'}, $h->{'outkey'}, $Utils::PIF::sn_addfields, $kind // $Utils::PIF::k_string, lc $inkey);
    ...
    # Sort the field values to match the order of the %card_field_specs entries, so that
    # they are ordered in the 1PIF file - this allows ordered custom fields.
    if (exists $cmeta->{'fields'}) {
	@{$cmeta->{'fields'}} = sort { $a->{'order'} <=> $b->{'order'} } @{$cmeta->{'fields'}};
    }
