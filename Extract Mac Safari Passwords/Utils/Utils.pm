...
sub summarize_import {
    my ($nouns, $n, $ignored, $ignored_reason) = @_;
    my ($noun1, $noun2) = ref($nouns) eq 'ARRAY' ? @$nouns : ($nouns, $nouns);

    verbose sprintf "Import:\n %4d %s discovered", $n, pluralize($noun1, $n);
    verbose sprintf ' %4d %s ignored (%s)', $ignored, pluralize('item', $ignored), $ignored_reason	 if $ignored;

    if ($Utils::Normalize::npre_explode) {
	verbose sprintf ' %4d %s were added by splitting %d original %s', 
	    $Utils::Normalize::npost_explode - $Utils::Normalize::npre_explode,
...
sub create_attachment {
    ...
    $attachdir ||= File::Spec->catfile(dirname($main::opts{'outfile'}), '1P_Attachments');
...
sub print_record {
    ...
    my @line;
    push @line, "title: $h->{'title'}";
    push @line, ' ' x 22 . "tags:  $tags";
    for my $f (@{$h->{'fields'}}) {
	push @line, sprintf "%22skey(%s): %s = %s",
	    '', $f->{'outkey'}, $f->{'inkey'},
	    ref $f->{'value'} eq 'HASH' ? join('', map { "\n" . ' ' x 26 . "$_: $f->{'value'}{$_}" } keys %{$f->{'value'}}) : $f->{'value'};
    }
    push @line, ' ' x 22 . "notes: " . unfold_and_chop($h->{'notes'} // '');
    debug join "\n", @line;
=cut
    for my $f (@{$h->{'fields'}}) {
