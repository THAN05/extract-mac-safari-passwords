...
# Handle folder redirection & OneDrive
if ($^O eq 'MSWin32') {
    my $wsh = new Win32::OLE 'WScript.Shell';
    $opts{'outfile'} = join('\\' , $wsh->SpecialFolders('Desktop'), '1P_import');
}
else {
    $opts{'outfile'} = join('/', $ENV{'HOME'}, 'Desktop', '1P_import');
}
...
# Converter name may be missing when --help or --info are used.
for (my $i = 0; $i < @ARGV; $i++) {
...
if (! $converter and ! $opts{'help'}) {
    Usage(1, "Command line does not specify a valid converter name.");
}
...
sub getInfo {
    ...
    $arg eq '' and
	bail "The --info option requires an argument of 'macos' or 'windows'.";

    $arg =~ /^macos|windows$/ or
	bail "The --info option currently only supports the arguments 'macos' or 'windows'.";
    my @converter_meta;
    my $suite_dir = $Bin;
