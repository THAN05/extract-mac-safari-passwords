...
The C<< --keepempty >> option prevents suppressing notes with empty bodies.
This can be useful if a note's title contains the meaningful data.

The C<< --titlefilter >> option allows supplying a Perl expression to modify a note's title.
For example, --filtertitle "s/\.txt$//" would remove the suffix ".txt" from the title.
...
sub do_init {
	...
	filelist_ok	=> 1,
	opts		=> [
			    [ q{-k or --keepempty          # keep notes with empty bodies }, 'keepempty|k' ],
			    [ q{-r or --recurse            # recurse into subdirectories },  'recurse|r' ],
			    [ q{      --titlefilter <RE>   # filter the note's title with regular expression RE },  'titlefilter=s' ],
			   ],
    }
}
...
sub do_import {
	...
	if (! $main::opts{'keepempty'} and $cmeta{'notes'} =~ /\A\s*\z/ms) {
	...
	# Run RE against title if requested
	if ($main::opts{'titlefilter'}) {
	    local $_ = $cmeta{'title'};
	    eval $main::opts{'titlefilter'};
	    $cmeta{'title'} = $_;
	}
