...
=item B<macOS>: Initially tested with version 1.23.0
...
When using the C<< --addfields >> option, custom fields of type "Hidden" will be concealed in 1Password.

Custom fields of type "Linked" will indicate the link's referent (i.e. the Username or Password field).
...
sub process_json {
	...
	#   type 1: hidden
	#   type 2: boolean
	#   type 3: linked (value 100: Username; value 101: Password)

	my $fnum;
	for my $field (@{$entry->{'fields'}}) {
	    $fnum++;
	    my ($label, $value, $type, $linkedId) = @{$field}{qw/name value type linkedId/};

	    $label ||= join "_", 'Unlabeled', $fnum;

	    #debug sprintf "%20s => %s\n",  $label, $value || '';
            next if (not defined $value or $value eq '') and $type != 3;		# value is undef for type 3

	    # When using --addfields, encode such that hidden text becomes concealed,
	    # and convert linkedID values to indicate the links to Username or Password.
	    # See Utils/Normalize.pm ("Hashref value indicates that attributes have been passed")
	    if ($type == 1) {			# hidden
		if ($main::opts{'addfields'}) {
		    $value = { value => $value, kind => $Utils::PIF::k_concealed };
		}
	    }
	    elsif ($type == 3) {		# linked
		$value = $linkedId == 100 ? 'link to Username' : 'link to Password';
	    }
