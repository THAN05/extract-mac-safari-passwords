# macOS / OS X local keychain converter
#
# Copyright 2014 Mike Cappella (mike@cappella.us)

package Converters::Keychain;
...
=head1 Keychain converter module

=head2 Platforms

=over

=item B<macOS>: Tested on macOS 10.9.5 to 10.14
...
Directly converts OS X / macOS local keychains to 1PIF for 1Password import.

=head2 Instructions

This converter can directly convert I<local> macOS keychains (no export is required).
It I<cannot> convert the iCloud Keychain. See Notes below for obtaining Safari passwords stored there.

The simplest method to convert a local keychain is to use the B<macOSConvertHelper>.
Otherwise, you will need to supply the command line with the I<full> path to the local keychain file that
you wish to convert (see Notes below for obtaining this path).

The converter will request the keychain's password.
Provide it when requested, and hit Enter.

=head2 Notes

B<< Getting Passwords from Safari 15 or newer >>

If you are using Safari version 15 or newer, you can export passwords natively to CSV.
Open Safari's C<< Preferences >>, go to the C<< Passwords >> tab, unlock it, and use the small pulldown menu at the bottom of the password list.
Select the C<< Export Passwords... >> item, and save the file to your Desktop.
You can import this CSV file with 1Password.

B<< Getting Passwords from Safari 12 to 14 stored in iCloud Keychain >>

If you wish to retrieve iCloud Keychain stored Safari passwords, use the B<Get_Safari_Passwords> AppleScript
included in the converter suite.

The video B<< Converting Safari Passwords.mp4 >> shows the procedure for running B<Get_Safari_Passwords> and converting the CSV file it generates.
It is available in the C<< Extra Help > Videos >> folder at the converter suite's download location.

B<< Getting Passwords from Safari 11 and previous >>

The older version of the script, B<Get_Safari11_Passwords> (which supported Sarari versions 11 and older) is still available in the Unsupported folder.

B<< Getting local keychain paths >>

To get a list of your I<local> keychain paths, in the Terminal app, use the command:

 security list-keychains

The list of local keychain paths will be output.
Copy the desired keychain path and paste it on the command line.  

B<< Old (unsupported) method for copying iCloud Keychain to a local keychain >>

Prior to macOS Sierra, it was possible to copy the iCloud Keychain data to a local keychain.
The method below no longer seems to work (and may not work even on older OS X / macOS releases), so the instructions below should be
considered obsolete and unsupported.

Conversion of the iCloud Keychain requires copying entries to a new I<local> keychain (there is no direct access to it).
However, this is problematic in macOS Sierra and above.
Some entries will not copy, and you will receive non-specific error messages.

To export an iCloud Keychain, you need to create a new local keychain and copy the contents from the iCloud Keychain.
Launch the B<Keychain Access> app, and select the C<< File > New Keychain >> menu item.
Name the keychain B<local-icloud>, and click the C<Create> button.
Enter a password and click C<OK>.
Under the C<Keychains> section, select the C<iCloud> keychain.
Now select all of the items in that keychain on the right, and copy them with C<< Edit > Copy >> (or ⌘ + C).
Select the B<local-icloud> keychain, and paste the entries with C<< Edit > Paste >> (or ⌘ + V) .
If macOS allows the paste (it may fail), you will be presented with I<many> authorization dialogs.
Enter your password repeatedly, as requested.
To simplify entering your password, open a new script document in B<Script Editor>, and copy/paste the following script:
...
replacing the word B<MYPASSWORD> with the password you just set for the keychain.
Run the AppleScript to automate performing the password entry.

You may then use the B<macOSConvertHelper> and select the B<local-icloud> keychain you just created.

Once the export and conversion has completed, you may delete the B<local-icloud> keychain in C<Keychain Access> (delete both References and Files).

You may now quit B<Keychain Access> and B<Script Editor>.

Copying items from the B<iCloud Keychain> to a local keychain sets the modified and created dates of the pasted entries to the time of the paste.
The original dates are not retained in the copies.

Some items from the iCloud Keychain cannot be copied.
You may have to work in batches to determine which can be copied and which cannot.
Resolving the reason for these failures is beyond the scope of this document.
...
my @rules = (
    ...
    DATA => [
		...
		{ c => sub { $itype eq 'note' and $_[0] =~ s/^.*<key>NOTE<\/key>\s+<string>(.+?)<\/string>.*$/$1/ms }, action => 'BREAK',
...
KEYCHAIN_ENTRY:
	...
	if ($contents =~ s/\Akeychain: (.*?)\n+(?=$|^keychain: ")//ms) {
	    ...
	    my $nNull = 0;
	    for (keys %entry) {
		$nNull++	if $entry{$_} eq '<NULL>';
	    }
	    if ($nNull == keys %entry) {
		$skipped++;
		debug "\t    skipping all NULL keychain entry";
		next KEYCHAIN_ENTRY	
	    }
		...
		if ($entry{'CLASS'} =~ /inet|genp/ and $rawdata =~ /^(\S+)\s+"ssgp.+"$/) {
		    ...
		    # don't bother trying to decode improbably long password data, except for notes
		    if ($entry{'type'} eq '"note"' or length $bytes <= $max_encrypted_password_length) {
...
sub KeyblobDecryption {
    ...
    length $plain == 0 and 
	bail "Failed to decrypt the dbkey - did you supply the correct password?";
