...
=item The order of the columns in your CSV file is unimportant
...
| **Email Account** | *Title*, *Type*, **Email Username**, **Email Password**, **SMTP Username**, **SMTP Password**, *Notes* | See Note 8 regarding Type. |
...
You may disable this with the C<< --noaddfields >> option.
...
my %card_field_specs = (
	...
	[ 'pop_type',		0, qr/^Type$/i, ],
	[ 'pop_username',	1, qr/^Email Username$/i, ],
	[ 'pop_password',	1, qr/^Email Password$/i, ],
	[ 'smtp_username',	1, qr/^SMTP Username$/i, ],
	[ 'smtp_password',	1, qr/^SMTP Password$/i, ],
    ]},
...
sub do_import {
    ...
    # Add extra columns as fields, by default.  This can be disabled via --noaddfields
    $main::opts{'addfields'}++	unless defined $main::opts{'addfields'};
    ...
    eval { $parsed = csv(
	    ...
	    diag_verbose => 1,
	    detect_bom => 1,
	    # attr sep_set is required rather than sep_char, since csv() calls header() which defaults to ',' and ';'.
	    #sep_char => $sepchar,
	    # But, very old upstream typo bug in Text::CSV_XS, and hence Text::CSV_PP, prevents sep_set from working.
	    # Fixed in the converter's local lib version
	    defined $sepchar ? ( sep_set => [ $sepchar ] ) : ( ),
