...
sub do_import {
	...
	foreach my $cardnode ($cardnodes->get_nodelist) {
	    ...
	    push @{$cmeta{'tags'}}, 'Stared'	if defined  $cardnode->getAttribute('star') and $cardnode->getAttribute('star') eq 'true';
	    if ($main::opts{'notimestamps'} and defined $cardnode->getAttribute('time_stamp')) {
