...
package Converters::Passwordsplus;
...
=head1 Passwords Plus converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 3.0020

=item B<Windows>: Initially tested with version 3.0.1 (232)
...
B<macOS>: Export its database to a file using the C<< Tools > Export >> menu item.
Click the C<Select Location> button, and enter the name B<pm_export.csv> in the C<Save As> area of the dialog,
and click the C<Where> button to navigate to your B<Desktop> folder.
Click the C<Save> button, then click the C<Export> button in the original dialog, and click the C<OK> button when the final dialog appears.

B<Windows>: Export its database to a file using the C<< File > Export ... >> menu.
Click C<OK> to acknowledge that your data will be exported in an unencrypted format.
Navigate to your B<Desktop> folder, enter the name B<pm_export.csv> in the C<File Name> area of the dialog, and click C<Save>.
...
sub do_import {
    ...
    # Don't set eol - getline() handles the change fine, and it breaks Windows.
    my $csv = Text::CSV->new ({
	    binary => 1,
	...
	if (shift @$row == 1) {
		...
		shift @$row; shift @$row;		# shift out the Value and Hidden cells
	    }
	}
    ...
    summarize_import('item', $n - 1, scalar (keys %categories), 'category templates');
    return \%Cards;
}
...
sub find_card_type {
    ...
    if (grep { $_->[0] =~ /^URL|Username|Password$/ } @$fieldlist ) {
