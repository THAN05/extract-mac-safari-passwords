...
=head1 Subsembly Wallet 4X/4W converter module

=head2 Platforms

=over

=item B<macOS>: Untested

=item B<Windows>: Initially tested with version 4.5.0.5095
...
Export its database as an HTML file using the C<< Wallet > Export HTML... >> menu.
Navigate to your B<Desktop> folder, and in the C<Save As> area, enter the file name B<pm_export>.
Click C<Save>.
Your exported data file will be on the Desktop with the name B<pm_export.html>.

You may now quit Wallet 4.

=head2 Notes

Wallet 4 does not export attached files or images, so they are not available to the converter.
Be sure to export your files from Wallet 4 before you remove it from your system.

Any Files or Pictures item in Wallet 4 will be imported as a 1Password Secure Note.
It will contain a reference to the original file name in the Notes section.

Favorites will not be retained.
Wallet 4 does not export unique references between an item and the list of favorites.
